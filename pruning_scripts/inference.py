#!/usr/bin/env python3
"""
Custom inference script for pruned models.
This script handles inference for pruned models in SageMaker endpoints.
"""
...
def model_fn(model_dir: str):
    """
    Load the pruned model for inference with improved error handling.
    ...
    """
    logger.info(f"Loading pruned model from {model_dir}")
    ...
    try:
        ...
        logger.info("Loading pruned PyTorch model...")
        model = AutoModelForSequenceClassification.from_pretrained(model_path)
        tokenizer = AutoTokenizer.from_pretrained(model_path)
        
        logger.info("Successfully loaded pruned model")
...
def predict_fn(input_data: Dict[str, Any], model_artifacts: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    logger.info(f"Running inference on {len(texts)} text(s) using {model_type} pruned model")
