#!/usr/bin/env python
# Fine-tuning solution for sentiment analysis using SageMaker and Hugging Face (Final version)
...
def upload_json_to_s3(s3_bucket, s3_prefix):
    ...
    for split_name in ["train", "validation"]:
        ...
        if os.path.exists(local_file):
            s3_path = f"s3://{s3_bucket}/{s3_prefix}/data/{split_name}.json"
            
            # Use AWS CLI for uploading
            result = os.system(f"aws s3 cp {local_file} {s3_path}")
            if result != 0:
...
def main():
    ...
    # Prepare and save dataset as JSON
    success = prepare_and_save_as_json(args.dataset_name, args.dataset_config, args.model_id)
    if not success:
        logger.error("Failed to prepare datasets")
        sys.exit(1)
    
    # Upload JSON datasets to S3
