#!/usr/bin/env python
# Fine-tuning solution for sentiment analysis using SageMaker and Hugging Face (Debug version)
...
def parse_args():
    ...
    parser.add_argument(
        "--role_arn",
        type=str,
        help="Custom IAM role ARN for SageMaker (optional)",
    )
    
    parser.add_argument(
        "--dry_run",
        action="store_true",
        help="Prepare everything but don't start the training job",
    )
...
def upload_to_s3(dataset_dict, s3_bucket, s3_prefix):
    ...
    for split_name, dataset in dataset_dict.items():
        ...
        # Save with simplified format for better compatibility
        try:
            # Remove any problematic metadata and save with basic format
            dataset_clean = dataset.remove_columns([col for col in dataset.column_names if col not in ['input_ids', 'attention_mask', 'labels']])
            dataset_clean.save_to_disk(local_path)
        except Exception as e:
            logger.warning(f"Failed to clean dataset, saving as-is: {e}")
        ...
        result = os.system(f"aws s3 cp {local_path} {s3_path} --recursive")
        if result != 0:
            logger.error(f"Failed to upload {split_name} dataset to S3")
            return None
...
def check_role_permissions(role_arn):
    """Check if the role has proper trust relationship with SageMaker"""
    try:
        ...
        logger.info(f"Role trust policy: {trust_policy}")
        
        # Check if SageMaker is in the trust policy
        ...
        if not sagemaker_trusted:
            logger.warning("Role does not trust sagemaker.amazonaws.com service")
            logger.info("The role needs to have a trust relationship with sagemaker.amazonaws.com")
        else:
            logger.info("Role has proper trust relationship with SageMaker")
            
        return sagemaker_trusted
...
def main():
    ...
    if not s3_paths:
        logger.error("Failed to upload datasets to S3")
        sys.exit(1)
    ...
    if args.role_arn:
        ...
        try:
            ...
            logger.info(f"Using default execution role: {role}")
        except Exception as e:
    ...
    if not check_role_permissions(role):
        logger.warning("Role may not have proper permissions for SageMaker")
    
    if args.dry_run:
        logger.info("Dry run mode - stopping before creating training job")
    ...
    try:
        ...
        logger.info("1. IAM role trust relationship issues")
        logger.info("2. Insufficient permissions")
        logger.info("3. Resource limits or quotas")
        logger.info("4. Instance type availability")
        raise
