#!/usr/bin/env python3
"""
Modified pruning script with a lower pruning amount.
...
def apply_structured_pruning(model, amount):
    ...
    for name, module in model.named_modules():
        if isinstance(module, torch.nn.Linear):
            ...
            # Skip pruning the classifier layer
            if "classifier" in name:
                logger.info(f"Skipping pruning for classifier layer: {name}")
                continue
                
            # Skip pruning if the layer is too small
            if len(norm) <= 10:
                logger.info(f"Skipping pruning for small layer: {name} with {len(norm)} neurons")
                continue
                
            # Use a lower pruning amount for attention layers
            if "attention" in name:
                local_amount = amount * 0.5  # Use half the pruning amount for attention layers
                num_to_prune = int(local_amount * len(norm))
                logger.info(f"Using reduced pruning amount ({local_amount:.2f}) for attention layer: {name}")
...
def main():
    ...
    parser.add_argument("--pruning-amount", type=float, default=0.1, help="Amount of weights to prune (0.0 to 1.0)")
    ...
    try:
        ...
        for model_key, info in model_info.items():
            try:
                ...
                # Save pruned model
                try:
                    pruned_model_dir = os.path.join(args.output_dir, f"{model_key}_pruned_light")
